...
// check to see if the letter of the obj selected matches any of the letters in the answer array
...
const Test = ({ location }) => {
  ...
  return state.loading ? (
    ...
    <Layout>
      ...
      <ul>
        {(state.test.answerArray || []).map(item => (
          <li key={item.id}>{item.letter}</li>
        ))}
      </ul>
      {!state.test.question ? null : <button onClick={() => handleClick()}>GAME</button>}
